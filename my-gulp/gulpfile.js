...
// scss
const scss = () => {
	return gulp.src(`${ srcFolder }/scss/style.scss`, { sourcemaps: !isBuild })
							.pipe(plumber(
								notify.onError({
									title: "SCSS",
									message: "Error <%= error.message %>"
...
// js
const js = () => {
	return gulp.src(`${ srcFolder }/js/script.js`, { sourcemaps: !isBuild })
							.pipe(plumber(
								notify.onError({
									title: "JS",
									message: "Error <%= error.message %>"
...
const mainTasks = gulp.parallel(copyFiles, html, scss, js)
